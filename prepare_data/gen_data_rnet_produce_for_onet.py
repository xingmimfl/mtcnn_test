...
for annotation in annotations:
    ...
    for scale in scales:
        ...
        scale_image = Variable(scale_image).float()
    ...
    for a_rect in rectangles:
        ...
        if np.max(iou) >= 0.65:
            ...
            f1.write("48/positive_hardmining/%s.jpg"%p_idx + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
            cv2.imwrite(save_file, resized_im)
            p_idx += 1                
        elif np.max(iou) >=0.4:
            if confidence >= 0.5:
                save_file = os.path.join(part_save_dir, "%s.jpg"%d_idx)
                f3.write("48/part_hardmining/%s.jpg"%d_idx + ' -1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
        elif np.max(iou) < 0.3: #--negative samples
            if confidence >= 0.6:
                save_file = os.path.join(neg_save_dir, "%s.jpg"%n_idx)
                f2.write("48/negative_hardmining/%s.jpg"%n_idx + ' 0 -1 -1 -1 -1\n')
                cv2.imwrite(save_file, resized_im)
                n_idx += 1                    
