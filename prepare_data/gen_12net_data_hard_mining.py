...
pnet = mtcnn.Pnet()
pnet.load_state_dict(torch.load("../pnet/face_detect_power_bbox2_iter_1484000_.pth"))
pnet.eval()
...
for annotation in annotations:
    ...
    for a_rect in rectangles:
        ...
        if np.max(iou) >= 0.65:
            ...
            f1.write("12/positive_hardmining/%s.jpg"%p_idx + ' 1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
            cv2.imwrite(save_file, resized_im)
            p_idx += 1                
        elif np.max(iou) >=0.4:
            if confidence >= 0.5:
                save_file = os.path.join(part_save_dir, "%s.jpg"%d_idx)
                f3.write("12/part_hardmining/%s.jpg"%d_idx + ' -1 %.2f %.2f %.2f %.2f\n'%(offset_x1, offset_y1, offset_x2, offset_y2))
                cv2.imwrite(save_file, resized_im)
                d_idx += 1
        elif np.max(iou) < 0.3: #--negative samples
            if confidence >= 0.9:
                save_file = os.path.join(neg_save_dir, "%s.jpg"%n_idx)
                f2.write("12/negative_hardmining/%s.jpg"%n_idx + ' 0 -1 -1 -1 -1\n')
                cv2.imwrite(save_file, resized_im)
                n_idx += 1                    
